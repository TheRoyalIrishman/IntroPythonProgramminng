...
#RQ2
    def go_to(self, direction):
        ...
        # checks if it's possible to go to a new location
        if direction in self.place.exits:
            newPlace, description = self.place.exits[direction]
            # sets current place as the new destination
            self.place = newPlace
...
#RQ3
    def talk_to(self, person):
        ...
        # checks if the person you're trying to talk to exists at a location
        if person in self.place.characters:
            # prints out the message associated with each character
            message = self.place.characters[person].talk()
            print('{} says: {}'.format(person, message))
...
#RQ4
    def take(self, thing):
        ...
        # checks if item is a valid thing that can be picked up
        if thing in self.place.things:
            # grabs item from location
    ...
    def check_backpack(self):
        ...
        # "if not" => if backpack is empty
        if not self.backpack:
            print('    there is nothing.')
        else:
            # prints out each item in backpack with its description
